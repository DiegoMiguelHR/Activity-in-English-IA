...
def definir_modelos_y_grids():
    ...
    """
    return {
        # --- Support Vector Classifier (analisis de distintos kernels) ---
        "SVC": (
            SVC(random_state=42),
            {
                "clf__C": [0.1, 1, 10],                  # penalización del error
                "clf__gamma": [0.01, 0.1, 1],            # radio de influencia del kernel
                "clf__kernel": ["rbf", "linear", "poly"],# análisis de kernels
            },
        ),
...
# --- K-Nearest Neighbors (evaluación de métricas de distancia) ---
        "KNN": (
            KNeighborsClassifier(),
            {
                "clf__n_neighbors": [3, 5, 7],                  # número de vecinos
                "clf__weights": ["uniform", "distance"],        # ponderación
                "clf__metric": ["euclidean", "manhattan"],      # métricas de distancia
            },
        ),
    }
...
def optimizar_y_evaluar(X, y, nombre, clasificador, param_grid, n_splits=5):
    ...
    # (el dataset está desbalanceado: ~77% vacía / ~23% ocupada)
    cv_interno = StratifiedKFold(n_splits=n_splits, shuffle=True, random_state=42)
