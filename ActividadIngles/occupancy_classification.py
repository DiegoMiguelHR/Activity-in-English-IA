    ...
    modelos = {
        ...
        "SVC": (
            SVC(random_state=42),
            {
                ...
                "clf__kernel": ["rbf", "linear", "poly"],# analisis de kernels
            },
        ),
        ...
        # --- K-Nearest Neighbors (evaluacion de metricas de distancia) ---
        "KNN": (
            KNeighborsClassifier(),
            {
                "clf__n_neighbors": [3, 5, 7],                  # numero de vecinos
                "clf__weights": ["uniform", "distance"],        # ponderacion
                "clf__metric": ["euclidean", "manhattan"],      # metricas de distancia
            },
        ),
    }
    return modelos
...
def optimizar_y_evaluar(X, y, nombre, clasificador, param_grid, n_splits=5):
    ...
    # StratifiedKFold: mantiene la proporcion de clases en cada pliegue
    # (el dataset esta desbalanceado: ~77% vacia / ~23% ocupada)
